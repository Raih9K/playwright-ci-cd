...
test.describe.serial("SKD Workflow Tests", () => {
  ...
  test("Create SKD Test", async ({ page }) => {
    ...
    // Select SKD type
    const skdTypeDropdown = page.getByRole("textbox", {
      name: "Search for a skd type",
      exact: true,
    });
    ...
    // Enter description
    const skdDescField = page.getByRole("textbox", {
      name: "Enter skd description",
    });
